...
export interface TrailingIconButtonProps
  extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'type'> {
  variant?: ButtonVariant;
  size?: ButtonSize;
  type?: 'button' | 'submit' | 'reset';
  /** Icon rendered after the label. Sized automatically by the button's `size`. */
  icon: ReactNode;
}
...
const SIZES: Record<ButtonSize, string> = {
  sm: cn(
    'h-[32px]',
    'pl-(--padding-sm) pr-(--padding-xs)',
    'gap-(--gap-2xs)',
    'text-(length:--font-size-web-body-sm)',
  ),
  md: cn(
    'h-[40px]',
    'pl-(--padding-md) pr-(--padding-sm)',
    'gap-(--gap-xs)',
    'text-(length:--font-size-web-body-md)',
  ),
  lg: cn(
    'h-[48px]',
    'pl-(--padding-xl) pr-(--padding-md)',
    'gap-(--gap-sm)',
    'text-(length:--font-size-web-body-md)',
  ),
};
